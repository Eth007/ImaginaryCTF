...
namespace iCTF_Shared_Resources.Managers
{
    public class SharedStatsManager
    {
        ...
        public static async Task<Stats> GetStats(DatabaseContext context, User user, bool dynamicScoring = false) {
            ...
            var challengesInfo = await context.Challenges.Where(x => x.State == 2).OrderByDescending(x => x.Solves.Count).Select(x => new ChallengeInfo { Challenge = x, SolvesCount = x.Solves.Count }).ToListAsync();
